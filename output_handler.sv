 ...
 * 				 LEDR[0] indicates if there's overflow, LED[3] shows the status of the enable signal
 * 				 (if lit, enable is on).
...
module output_handler(
	...
	// Declare internal signals for values being displayed
	...
	// Assign each 8-bit value into 2 4-bit nibbles
	...
	// Result display logic (blank if reset on or enable off)
	always_comb begin
		if (reset_n == 1'b0)
			result_display = 8'b0;
		else if (enable == 1'b0)
	...
	assign LEDR [9:4] = 8'b0;
	assign LEDR [2:1] = 8'b0;
